...
<%
    ...
    try {
        ...
        // 사용자가 이미 투표했는지 확인
        String checkVoteQuery = "SELECT COUNT(*) AS vote_count FROM votes WHERE user_id = ? AND post_id = ?";
        checkVoteStmt = conn.prepareStatement(checkVoteQuery);
        checkVoteStmt.setString(1, userId);
        checkVoteStmt.setInt(2, postId);
        ResultSet checkRs = checkVoteStmt.executeQuery();

        boolean hasVoted = false;
        if (checkRs.next()) {
            hasVoted = checkRs.getInt("vote_count") > 0;
        }
        checkRs.close();
        checkVoteStmt.close();
        ...
        if (hasVoted) {
            json.append("\"status\":\"error\",");
            json.append("\"message\":\"이미 이 게시글에 대해 투표하셨습니다.\"");
            json.append("}");
            out.print(json.toString());
            return;
        }
        ...
        // 고유 제약 조건 위반 시 처리 (이미 투표한 경우)
        json.setLength(0);
        json.append("{\"status\":\"error\",\"message\":\"이미 이 게시글에 대해 투표하셨습니다.\"}");
        out.print(json.toString());
    } catch (Exception e) {
        e.printStackTrace();
        json.setLength(0);
